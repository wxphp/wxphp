            ...
            switch($standard_parameter_type)
            {
                ...
                case    "characters":
                {
                    switch($declaration_modifier)
                    {
                        ...
                        case "reference": //char&
                            $parameters .= tabs(1) . "long " . $declaration[$parameter_names][$parameter_index] . $declaration_index . ";\n";
                            $parameters .= tabs(1) . "zval " . $declaration[$parameter_names][$parameter_index] . $declaration_index . "_ref;\n";
                            break;
                ...
                {
                    $parameters .= "zval* " . $declaration[$parameter_names][$parameter_index] . $declaration_index . ";\n";
                    $parameters .= tabs(1) . "{$plain_type}* object_pointer{$declaration_index}_{$parameter_index} = 0;\n";
                    break;
                }
                default:
                    ob_clean();
...
function function_called_overload($method_definitions, $method_name, $class_name=null)
{
    ...
    foreach($method_definitions as $declaration_index=>$declaration)
    {
        ...
        {
            $clause .= " && (current_object_type == "
                . "PHP_".strtoupper($declaration["override_class"])."_TYPE"
            ;

            $override_derivations = derivationsOfClass($declaration["override_class"]);

            if(count($override_derivations) > 0)
            {
                foreach($override_derivations as $override_class_child=>$isset)
                {
                    $clause .= " || "
                        . "current_object_type == "
                        . "PHP_".strtoupper($override_class_child)."_TYPE"
                    ;
                }
            }
        ...
        $called_overload .= tabs(1) . "overload$declaration_index:\n";
        $called_overload .= tabs(1) . "if(!already_called && $clause)\n";
        $called_overload .= tabs(1) . "{\n";
        ...
        for($parameter_index=0; $parameter_index<$parameters_count; $parameter_index++)
        {
            ...
            switch($standard_parameter_type)
            {
                ...
                case "object":
                {
                    ...
                    $object_retrieve_code .= tabs(3) . "if(arguments_received >= ".($parameter_index+1)."){\n";
                    $object_retrieve_code .= tabs(4) . "if(Z_TYPE_P(".$declaration[$parameter_names][$parameter_index] . $declaration_index.") == IS_OBJECT)\n";
                    $object_retrieve_code .= tabs(4) . "{\n";
                    $object_retrieve_code .= tabs(5) . "wxphp_object_type argument_type = Z_{$argument_plain_type}_P(".$declaration[$parameter_names][$parameter_index] . $declaration_index.")->object_type;\n";
                    ...
                    {
                        $object_retrieve_code .= tabs(6) . "goto overload".($declaration_index+1).";\n";
                    }
                    else
                    {
                        $object_retrieve_code .= tabs(6) . "zend_error(E_ERROR, \"Parameter '".$declaration[$parameter_names][$parameter_index]."' could not be retreived correctly.\");\n";
                    }

                    $object_retrieve_code .= tabs(5) . "}\n";
                    $object_retrieve_code .= tabs(4) . "}\n";
                    $object_retrieve_code .= tabs(4) . "else if(Z_TYPE_P(".$declaration[$parameter_names][$parameter_index] . $declaration_index.") != IS_NULL)\n";
                    $object_retrieve_code .= tabs(4) . "{\n";

                    if(($declaration_index+1) != $declarations_count && $declarations_count > 1)
                    {
                        $object_retrieve_code .= tabs(5) . "goto overload".($declaration_index+1).";\n";
                    }
                    else
                    {
                        $object_retrieve_code .= tabs(5) . "zend_error(E_ERROR, \"Parameter '".$declaration[$parameter_names][$parameter_index]."' not null, could not be retreived correctly.\");\n";
                    }

                    $object_retrieve_code .= tabs(4) . "}\n";
                    $object_retrieve_code .= tabs(3) . "}\n\n";
                    break;
                }
            }
        ...
        $called_overload .= tabs(2) . "#ifdef USE_WXPHP_DEBUG\n";
        $called_overload .= tabs(2) . "php_printf(\"Parameters received %d\\n\", arguments_received);\n";
        $called_overload .= tabs(2) . "php_printf(\"Parsing parameters with '$zend_parse_parameters_string' (".trim($zend_parse_parameters, ", ").")\\n\");\n";
        $called_overload .= tabs(2) . "#endif\n\n";

        if($required_parameters == 0 && $parameters_count < 1)
        {
            $called_overload .= tabs(2) . "overload{$declaration_index}_called = true;\n";
            $called_overload .= tabs(2) . "already_called = true;\n";
        }
        else
        {
            $called_overload .= tabs(2) . "char parse_parameters_string[] = \"$zend_parse_parameters_string\";\n";
            $called_overload .= tabs(2) . "if(zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, arguments_received, parse_parameters_string, ".trim($zend_parse_parameters, ", ")." ) == SUCCESS)\n";
            $called_overload .= tabs(2) . "{\n";
            $called_overload .= $object_retrieve_code;
            $called_overload .= tabs(3) . "overload{$declaration_index}_called = true;\n";
            $called_overload .= tabs(3) . "already_called = true;\n";

            //Just get variables not originally parsed as zvals since they are references
            if($references_found)
            {
                $called_overload .= "\n".tabs(3)."char parse_references_string[] = \"$references_parameters_string\";\n";
                $called_overload .= tabs(3) . "zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, arguments_received, parse_references_string, ".trim($references_parameters, ", ")." );\n";
            }
...
function function_return($method_definitions, $method_name, $class_name=null, $is_constructor=false)
{
    ...
    foreach($method_definitions as $declaration_index=>$declaration)
    {
        ...
        $return_called_overload .= tabs(2) . "switch(arguments_received)\n";
        $return_called_overload .= tabs(2) . "{\n";

        for($required_parameters; $required_parameters<=$parameters_count; $required_parameters++)
        {
            $return_called_overload .= tabs(3) . "case $required_parameters:\n";
                    ...
                    case "object":
                    {
                        switch($return_modifier)
                        {
                            ...
                            case "const_pointer":
                                ...
                                $return_called_overload .= tabs(5) . "Z_{$return_type}_P(return_value)->native_object = ({$return_type}_php*) value_to_return{$required_parameters};\n";
                                $return_called_overload .= tabs(4) . "}\n\n";
                                ...
                                if(!$declaration["static"] && $class_name)
                                {
                                    ...
                                    $return_called_overload .= tabs(5) . "references->AddReference(return_value, \"$class_name_ex::$method_name at call 5 with $required_parameters argument(s)\");\n";
                                    $return_called_overload .= tabs(4) . "}\n";
                                }
                            ...
                            case "reference":
                                ...
                                if(!$declaration["static"] && $class_name)
                                {
                                    ...
                                    $return_called_overload .= tabs(5) . "references->AddReference(return_value, \"$class_name_ex::$method_name at call 6 with $required_parameters argument(s)\");\n";
                                    $return_called_overload .= tabs(4) . "}\n";
                                }
                            ...
                            case "none":
                                ...
                                else
                                {
                                    ...
                                    $return_called_overload .= tabs(4) . "zo_{$return_type}* zo{$required_parameters} = Z_{$return_type}_P(return_value);\n";
        ...
        case "object":
        {
            switch($return_modifier)
            {
                ...
                case "none":
                    ...
                    $call_code .= tabs($t) . "zo_{$return_type}* zo{$required_parameters} = Z_{$return_type}_P(return_value);\n";
