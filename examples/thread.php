    ...
    function Entry()
    {
        ...
        foreach($this->documents as $input)
        {
            ...
            $output = $this->output_dir
                . "/"
                . str_ireplace(".pdf", ".swf", $file_name)
            ;
...
class ConvertFrame extends wxFrame {
    ...
    function __construct( $parent=null ){
        ...
        $this->fileNavigator = new wxGenericDirCtrl(
            $this,
            wxID_ANY,
            wxEmptyString,
            wxDefaultPosition,
        ...
        $this->m_lblDocumentsToConvert = new wxStaticText(
            $this,
            wxID_ANY,
        ...
        $bSizer4->Add(
            $this->m_lblDocumentsToConvert,
            1,
            wxALIGN_CENTER_VERTICAL|wxALL,
            5
        );

        $this->m_clear = new wxButton(
            $this,
            wxID_ANY,
            "Clear List",
            wxDefaultPosition,
        ...
        $this->m_pdfList = new wxCheckListBox(
            $this,
            wxID_ANY,
            wxDefaultPosition,
            wxDefaultSize,
        ...
        $this->m_lblOutputDir = new wxStaticText(
            $this,
            wxID_ANY,
            "Output:",
            wxDefaultPosition,
        ...
        $this->m_outpurDir = new wxDirPickerCtrl(
            $this,
            wxID_ANY,
            wxEmptyString,
        ...
        // Connect Events
        $this->fileNavigator->Connect(
            wxEVT_KEY_DOWN,
            array($this, "onFileNagivationKeyDown")
        );
        $this->fileNavigator->GetTreeCtrl()->Connect(
            wxEVT_LEFT_DCLICK,
            array($this, "onFileNavigationDClick")
        );
        $this->m_clear->Connect(
            wxEVT_COMMAND_BUTTON_CLICKED,
            array($this, "onClearClick")
        );
        $this->m_pdfList->Connect(
            wxEVT_COMMAND_CHECKLISTBOX_TOGGLED,
            array($this, "onPdfListItemToggled")
        );
        $this->m_convert->Connect(
            wxEVT_COMMAND_BUTTON_CLICKED,
            array($this, "onConvertClick")
        );
    ...
    {
        if(
            is_file($this->fileNavigator->GetPath())
            &&
            "".$this->m_pdfList->FindString(
                $this->fileNavigator->GetPath()
            ).""
            ==
            "-1"
        )
        {
            $this->m_pdfList->InsertItems(
                array($this->fileNavigator->GetPath()),
                $this->m_pdfList->GetCount()
            );
    ...
    function onClearClick( $event ){
        if(
            wxMessageBox(
                "Are you sure you want to clear the added files?",
                "",
                wxYES_NO
            )
            ==
    ...
    function onConvertClick( $event ){
        ...
        else
        {
            ...
            $this->document_converter->SetDocuments(
