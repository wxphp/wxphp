...
PHP_FUNCTION(php_wxDynamicCast)
{
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS() TSRMLS_CC, parse_parameters, &_argObj0 , &_argStr0 , &_argStr0_len ) == SUCCESS){
		...
		if(valid){
			...
			if(!strcmp(_argStr0, "wxWebViewEvent")){
				object_init_ex(return_value, php_wxWebViewEvent_entry);
				wxWebViewEvent* ret = wxDynamicCast(_ptrObj0, wxWebViewEvent_php);
				long id_to_find = zend_list_insert(ret, le_wxWebViewEvent);
				add_property_resource(return_value, _wxResource, id_to_find);
				return;
			}
			if(!strcmp(_argStr0, "wxWebView")){
				object_init_ex(return_value, php_wxWebView_entry);
				wxWebView* ret = wxDynamicCast(_ptrObj0, wxWebView_php);
				long id_to_find = zend_list_insert(ret, le_wxWebView);
				add_property_resource(return_value, _wxResource, id_to_find);
				return;
			}
		}
