 ...
 * input the serialized output produced by gccxml.
 * 
 * Usage:
 * 
...
//Load event constants not returned by gccxml
$event_constants_array = file("event_consts.txt");
foreach($event_constants_array as $event_constant)
{
	$defConsts[trim($event_constant)] = 1;
}

//Load classes parsed by gccxml
...
if(isset($argv[2]) && file_exists($argv[2]))
{
	...
	unset($defConsts['wxGRID_TOPCTRL']);
...
//For classes that doesn't have a file
...
<?=$retType ?> <?=$className?>_php::<?=$methodName?>(<?=$args?>)<?=$const?>
{
	...
	<?
		...
		foreach($argos as $arK => $arV){
			...
			if( "" . strstr($methodArgs[0][$arK+1], "bool") . "" == "" &&
				"" . strstr($methodArgs[0][$arK+1], "long") . "" == "" &&
				"" . strstr($methodArgs[0][$arK+1], "int") . "" == "" &&
				"" . strstr($methodArgs[0][$arK+1], "short") . "" == "" &&
				"" . strstr($methodArgs[0][$arK+1], "double") . "" == "" && 
				"" . strstr($methodArgs[0][$arK+1], "float") . "" == "" ||
				"" . strstr($methodArgs[0][$arK+1], "int*") . "" != ""
			 )
			 {
				...
				if("" . strstr($methodArgs[0][$arK+1], "const") . "" != "")
					$explicit_cast .= "const ";
				
				$explicit_cast .= "long int";
				
				if("" . strstr($methodArgs[0][$arK+1], "*") . "" != "")
					$explicit_cast .= ") ";
				else
					$explicit_cast .= ") &";
			 }
	?>
	zval argLng<?=$arK?>;
	ZVAL_LONG(&argLng<?=$arK?>,<?=$explicit_cast?>arg<?=$arK?>);
	args[<?=$arK?>]= &argLng<?=$arK?>;
	<?
		}
	?>
...
<?				}
		...
		if (valid) 
		{
			if(_argObj<?=$i?>->type==IS_OBJECT && zend_hash_find(Z_OBJPROP_P(_argObj<?=$i?>), _wxResource , sizeof(_wxResource),  (void **)&tmp) == SUCCESS)
			{
				...
				if (!_ptrObj<?=$i?> <?php if(trim($typeVerifierStr) != "") {print "|| ($typeVerifierStr)";} ?>)
					valid = 0;
			}
...
<?
		...
		if(valid && _argObj<?=$L+$i?>)
		{
			if (_argObj<?=$L+$i?>->type==IS_OBJECT && zend_hash_find(Z_OBJPROP_P(_argObj<?=$L+$i?>), _wxResource , sizeof(_wxResource),  (void **)&tmp) == SUCCESS) 
			{
				...
				if (!_ptrObj<?=$L+$i?> <?php if(trim($typeVerifierStr) != "") {print "|| ($typeVerifierStr)";} ?>)
					valid = 0;
			}
