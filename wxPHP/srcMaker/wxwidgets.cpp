...
zend_class_entry *php_wxBrushBase_entry;
int le_wxBrushBase;
zend_class_entry *php_wxWizard_entry;
int le_wxWizard;
zend_class_entry *php_wxWizardBase_entry;
int le_wxWizardBase;
...
PHP_FUNCTION(php_wxDynamicCast){
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS() TSRMLS_CC, "z!s!",  &_argObj0 , &_argStr0 , &_argStr0_len ) == SUCCESS){
		...
		if(valid){
			...
			if(!strcmp(_argStr0, "wxBrushBase")){
				object_init_ex(return_value, php_wxBrushBase_entry);
				wxBrushBase* ret = wxDynamicCast(_ptrObj0, wxBrushBase_php);
				long id_to_find = zend_list_insert(ret, le_wxBrushBase);
				add_property_resource(return_value, _wxResource, id_to_find);
				return;
			}
			...
			if(!strcmp(_argStr0, "wxWizardBase")){
				object_init_ex(return_value, php_wxWizardBase_entry);
				wxWizardBase* ret = wxDynamicCast(_ptrObj0, wxWizardBase_php);
				long id_to_find = zend_list_insert(ret, le_wxWizardBase);
				add_property_resource(return_value, _wxResource, id_to_find);
				return;
			}
...
INIT_CLASS_ENTRY(cf, PHP_wxBrushBase_NAME , php_wxBrushBase_functions);
php_wxBrushBase_entry = zend_register_internal_class(&cf TSRMLS_CC);
le_wxBrushBase = zend_register_list_destructors_ex(php_wxBrushBase_destruction_handler, NULL, le_wxBrushBase_name, module_number);
...
INIT_CLASS_ENTRY(cf, PHP_wxWizardBase_NAME , php_wxWizardBase_functions);
php_wxWizardBase_entry = zend_register_internal_class(&cf TSRMLS_CC);
le_wxWizardBase = zend_register_list_destructors_ex(php_wxWizardBase_destruction_handler, NULL, le_wxWizardBase_name, module_number);
